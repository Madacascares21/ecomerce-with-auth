...
export default function CategoryList({
  ...
  sampleProducts.forEach((product) => {
    product.variants.forEach((variant) => {
      const sizeAttr = variant.attributes.find((attr) => attr.Key === "Size");
      if (sizeAttr) sizesSet.add(sizeAttr.Value);
      if (variant.price.amount > maxPrice) maxPrice = variant.price.amount;
    });
  });
  ...
  const filteredAndSortedProducts = sampleProducts
    ...
    .sort((a, b) => {
      const getLowestPrice = (p: any) =>
        Math.min(...p.variants.map((v: any) => v.price.amount));
      if (sortOption === "price-asc")
        return getLowestPrice(a) - getLowestPrice(b);
  ...
  return (
    <div className="min-h-screen  bg-gray-50">
      ...
      <Container className="container mx-auto px-4 py-6">
        <div className="grid lg:grid-cols-[280px_1fr] gap-8">
          <div className="space-y-6">
            <h1 className="hidden lg:block text-2xl font-bold mb-6">Filtru</h1>
            <PriceFilter
              {...{
                priceRange,
                setPriceRange,
